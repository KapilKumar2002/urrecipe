...
class Cling(object):
    ...
    def __call__(self, environ, start_response):
        ...
        prezipped = "gzip" in environ.get("HTTP_ACCEPT_ENCODING", []) and path.exists(
            full_path + ".gz"
        )
        if prezipped:
            full_path += ".gz"
        ...
        try:
            ...
            if if_modified and (
                rfc822.parsedate(if_modified) >= rfc822.parsedate(last_modified)
            ):
    ...
    def _is_under_root(self, full_path):
        ...
        if (path.abspath(full_path) + path.sep).startswith(
            path.abspath(self.root) + path.sep
        ):
            return True
        else:
            return False
    ...
    def _add_headers(self, headers, path, content_type):
        ...
        for config in getattr(self, "headers", []):
            if (
                path.startswith(config.get("prefix", DEFAULT))
                or content_type == config.get("type", DEFAULT)
                or path.endswith(config.get("ext", DEFAULT))
            ):
...
class GenshiMagic(StringMagic):
    ...
    def body(self, environ, full_path):
        ...
        return [template.generate(**variables).render("html", doctype="html")]
...
def test():
    ...
    magics = (
        StringMagic(title="String Test"),
        KidMagic(title="Kid Test"),
        GenshiMagic(title="Genshi Test"),
    )
